...
pickle.dump(short_rfc,open('speech_classifier_short.p','w'))
print 'Model speech_classifier_short.p saved'
...
print 'Hyperparametrizing over long'


for p,params in enumerate(param_list):
	print p
        params['n_jobs'] = 28
        splitGenerator = yieldIdxs(foldFileList1,idxs1)
        f1_list = []
        precision_list = []
        recall_list = []
        for i in xrange(len(foldFileList1)):
...
print 'Training global model for long with these parameters.'
daylong_rfc_near = RandomForestClassifier(**param_list[best_param_idx1])
daylong_rfc_near.fit(X1,y1)

pickle.dump(daylong_rfc_near,open('speech_classifier_long.p','w'))

print 'Model speech_classifier_long.p saved.'


# learn a classifier for speech/nonspeech now
